...
export default function StrengthTrainingSection({
  ...
  return (
    <div className="space-y-4">
        ...
        >
          ...
          <div className="space-y-2 mb-3">
            {exercise.sets.map((set, setIndex) => (
              <div key={`set-${exerciseIndex}-${setIndex}`} className="flex items-center gap-2">
                <span className="text-xs font-medium text-gray-600 w-8">Set {setIndex + 1}</span>
                <input
                  type="number"
                  value={set.reps || ''}
      ...
      {errors.exercises && <p className="text-sm text-red-600 font-medium">{errors.exercises}</p>}
    </div>
  );
}
