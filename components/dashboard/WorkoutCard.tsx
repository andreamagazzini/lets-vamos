...
export default function WorkoutCard({ workout, onEdit, onDelete }: WorkoutCardProps) {
  ...
  const isCardio = workout.type === 'Run' || workout.type === 'Bike' || workout.type === 'Swim';
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-start gap-3 flex-1">
          <div className={`p-2 rounded-lg bg-white/60 ${colors.icon} flex-shrink-0`}>{icon}</div>
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-success text-lg">✓</span>
              <span className="font-bold text-sm">{workout.type}</span>
              <span className="text-xs opacity-70">{formatDate(workout.date)}</span>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 mt-2 text-xs">
              {workout.amount &&
                (() => {
                  ...
                  const isDistanceUnit = ['km', 'miles', 'm'].includes(unit);
                  const Icon = isDistanceUnit ? MapPin : Activity;
                  return (
              ...
              {!workout.amount && workout.distance && (
                <div className="flex items-center gap-1">
            ...
            {isCardio && (
              <div className="flex flex-wrap items-center gap-3 mt-2 text-xs">
                ...
                {workout.type === 'Swim' && workout.poolLength && (
                  <span className="opacity-70">({workout.poolLength}m pool)</span>
                )}
            ...
            {workout.notes && (
              <div className="text-xs mt-2 opacity-80 italic">"{workout.notes}"</div>
            )}
          </div>
        </div>
