...
export default function PlanSettingsModal({
  ...
  const [settings, setSettings] = useState<GroupPlanSettings>(currentSettings || DEFAULT_SETTINGS);
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setSettings((prev) => ({
                  ...prev,
                  displayStyle: e.target.value as 'compact' | 'expanded' | 'detailed',
                }))
              }
            ...
            <p className="mt-1 text-xs text-gray-500">
              {settings.displayStyle === 'compact' && 'Minimal info, icons only'}
              {settings.displayStyle === 'expanded' && 'Icons + type name'}
              {settings.displayStyle === 'detailed' && 'Icons + type + duration/distance'}
            </p>
          </div>

          {/* Color Theme */}
          <div>
            <label htmlFor="color-theme" className="block text-sm font-semibold text-gray-700 mb-2">
              Color Theme
            </label>
            <select
              id="color-theme"
              ...
              onChange={(e) =>
                setSettings((prev) => ({
                  ...prev,
                  colorTheme: e.target.value as 'default' | 'minimal' | 'vibrant',
                }))
              }
              className="w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-primary transition-colors"
            >
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <span className="text-sm font-semibold text-gray-700">Show Icons</span>
                <p className="text-xs text-gray-500">Display workout type icons</p>
              </div>
              <button
                type="button"
            ...
            <div className="flex items-center justify-between">
              <div>
                <span className="text-sm font-semibold text-gray-700">Show Details</span>
                <p className="text-xs text-gray-500">Display duration and distance</p>
              </div>
              <button
                type="button"
            ...
            <div className="flex items-center justify-between">
              <div>
                <span className="text-sm font-semibold text-gray-700">Highlight Today</span>
                <p className="text-xs text-gray-500">Emphasize today's workouts</p>
              </div>
              <button
                type="button"
