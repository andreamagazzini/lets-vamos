import { getRecentWorkouts, getWorkoutsByMemberId } from '@/lib/dashboard-utils';
    ...
    <div className="card-modern">
      ...
      <div className="space-y-6">
        {members.length === 0 ? (
          ...
          <div className="space-y-6">
            {members.map((member) => {
              const memberWorkouts = getWorkoutsByMemberId(recentWorkouts, member.userId);
              return (
                <div
                  key={member.userId}
                  ...
                  {memberWorkouts.length > 0 ? (
                    <div className="space-y-3">
                      {memberWorkouts.map((workout) => (
                        <WorkoutCard
                          key={workout._id?.toString() || workout.date}
                          workout={workout}
                          onEdit={() => onEditWorkout(workout)}
                          onDelete={() => onDeleteWorkout(workout._id?.toString() || '')}
                        />
                      ))}
                    </div>
                  ) : (
                    <div className="text-gray-400 body-sm">No workouts logged this week</div>
                  )}
                </div>
              );
            })}
